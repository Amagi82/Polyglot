...
class StringFormatter(
    ...
    companion object {
        val defaultFormatters: List<StringFormatter> by lazy {
            listOf(
                StringFormatter(arg = "\n") { _, _ -> "\\n" },
                StringFormatter(arg = "\'", platforms = Platform.AndroidOnly) { _, _ -> "\\'" },
                StringFormatter(arg = "%s", isIndexed = true, platforms = Platform.AndroidOnly) { index, _ -> "%$index\$s" },
                StringFormatter(arg = "%d", isIndexed = true, platforms = Platform.AndroidOnly) { index, _ -> "%$index\$d" },
                StringFormatter(arg = "%f", isIndexed = true, platforms = Platform.AndroidOnly) { index, _ -> "%$index\$f" },
                StringFormatter(arg = "%s", platforms = Platform.iosOnly) { _, _ -> "%@" },
                StringFormatter(arg = "\"", platforms = Platform.iosOnly) { _, isXml -> if (isXml) "\\\"" else "\"" })
        }
    }
}
